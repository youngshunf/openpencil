...
// ---------------------------------------------------------------------------
// Icon name → SVG path dictionary (headless/file-mode resolution)
//
// This is a self-contained port of the web app's icon dictionary
// (apps/web/src/services/ai/icon-dictionary.ts). It lets the pen-mcp server
// resolve icon NAMES emitted by an LLM (e.g. a `path` node named "MailIcon")
// into real verified SVG `d` data WITHOUT a browser or network — so designs
// generated by a headless agent show correct icons instead of placeholder
// boxes/hexagons.
//
// Sets, in priority order (first writer wins per key):
//   1. hand-picked Lucide-style icons (correct fill/stroke styling)
//   2. Lucide (1700+ icons, stroke) + its aliases
//   3. common AI aliases (login → log-in, etc.)
//   4. Feather (286 icons, stroke) — fallback
//   5. curated brand/social logos from Simple Icons (fill) — for social login
// ---------------------------------------------------------------------------
...
function iconifyBodyToPathD(body: string): string | null {
  ...
  // After concatenation, a leading lowercase "m" would become relative to the
  // previous sub-path's endpoint. Force absolute for every sub-path after the first.
  for (let i = 1; i < parts.length; i++) {
    if (parts[i].startsWith('m')) parts[i] = 'M' + parts[i].slice(1);
  }
  return parts.join(' ');
}

// ---------------------------------------------------------------------------
// Populate from Iconify sets (first writer wins per key)
// ---------------------------------------------------------------------------

(function initIconSets() {
  // Lucide — primary icon set (1700+ icons, stroke)
  ...
  for (const [name, icon] of Object.entries(lucideIcons)) {
    ...
    if (!d) continue;
    const entry: IconEntry = { d, style: 'stroke', iconId: `lucide:${name}` };
  ...
  // Common AI aliases (names LLMs use that aren't literal Lucide slugs)
  ...
  for (const [alias, target] of Object.entries(commonAliases)) {
    ...
    if (targetEntry && !ICON_PATH_MAP[alias]) ICON_PATH_MAP[alias] = targetEntry;
  }
  ...
  for (const [name, icon] of Object.entries(featherIcons)) {
    const d = iconifyBodyToPathD(icon.body);
    if (!d) continue;
    const entry: IconEntry = { d, style: 'stroke', iconId: `feather:${name}` };
    if (!ICON_PATH_MAP[name]) ICON_PATH_MAP[name] = entry;
    const normalized = name.replace(/-/g, '');
    if (!ICON_PATH_MAP[normalized]) ICON_PATH_MAP[normalized] = entry;
  }

  // Brand / social logos — curated subset of Simple Icons (fill style).
  // Powers social-login buttons (WeChat / QQ / Google / Apple / ...). We do
  // NOT load the full 3600-icon set: brand slugs would pollute the generic
  // name space. Only friendly-name → slug pairs we explicitly want resolve.
  const simpleIcons = (simpleIconsData as { icons: Record<string, { body: string }> }).icons;
  ...
  for (const [alias, slug] of Object.entries(brandAliases)) {
    if (ICON_PATH_MAP[alias]) continue; // never override a real UI icon
    const icon = simpleIcons[slug];
    if (!icon) continue;
    const d = iconifyBodyToPathD(icon.body);
    if (!d) continue;
    ICON_PATH_MAP[alias] = { d, style: 'fill', iconId: `simple-icons:${slug}` };
  }
})();
...
/** Longest known key the name STARTS with (prefix, ≥4 chars, ≥50% coverage). */
...
/** Longest known key the name ENDS with (suffix, ≥4 chars, ≥50% coverage). */
...
/** Resolve an icon by a free-form name. Exact → prefix → suffix fallback. */
export function lookupIconByName(name: string): IconEntry | null {
  const normalized = name
    .replace(/^[a-z-]+:/i, '') // strip icon set prefix (lucide:, simple-icons:, ...)
    .replace(/[-_\s/]+/g, '')
    .replace(/icon$/i, '')
    .toLowerCase();
